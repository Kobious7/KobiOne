...
public class SpecialUI : GMono
{
    ...
    public void ShowSpecial(InventoryEquip item)
    {
    ...
    private void Click()
    {
        if(!onSelectObject.gameObject.activeSelf)
        {
            if(Time.time - lastTimeClick < doubleClickTheshold)
            {
                ...
                EquipDetailsUI.Instance.ShowDetails(InfiniteMapManager.Instance.Inventory.SpecialList[index]);
            }
        ...
        if(SpecialUISpawner.Instance.SelectedItem != null) SpecialUISpawner.Instance.SelectedItem.gameObject.SetActive(true);
        onSelectObject.gameObject.SetActive(false);

        SpecialUISpawner.Instance.SelectedItem = onSelectObject;
    }
}