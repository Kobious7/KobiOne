...
public class ArmwearUI : GMono
{
    ...
    public void ShowArmwear(InventoryEquip item)
    {
    ...
    private void Click()
    {
        if(!onSelectObject.gameObject.activeSelf)
        {
            if(Time.time - lastTimeClick < doubleClickTheshold)
            {
                ...
                EquipDetailsUI.Instance.ShowDetails(InfiniteMapManager.Instance.Inventory.ArmwearList[index]);
            }
        ...
        if(ArmwearUISpawner.Instance.SelectedItem != null) ArmwearUISpawner.Instance.SelectedItem.gameObject.SetActive(true);
        onSelectObject.gameObject.SetActive(false);

        ArmwearUISpawner.Instance.SelectedItem = onSelectObject;
    }
}