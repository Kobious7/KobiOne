...
public class ArmorUI : GMono
{
    ...
    public void ShowArmor(InventoryEquip item)
    {
    ...
    private void Click()
    {
        if(!onSelectObject.gameObject.activeSelf)
        {
            if(Time.time - lastTimeClick < doubleClickTheshold)
            {
                ...
                EquipDetailsUI.Instance.ShowDetails(InfiniteMapManager.Instance.Inventory.ArmorList[index]);
            }
        ...
        if(ArmorUISpawner.Instance.SelectedItem != null) ArmorUISpawner.Instance.SelectedItem.gameObject.SetActive(true);
        onSelectObject.gameObject.SetActive(false);

        ArmorUISpawner.Instance.SelectedItem = onSelectObject;
    }
}