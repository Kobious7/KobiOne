...
public class SkillDetailUIActionBtns : GMono
{
    ...
    public void AddClickListeners(SkillNode skill, int treeIndex)
    {
        if(skill.Level > 0)
        {
            ...
            if(skill.skillSO is ActiveSkillSO)
            {
                ...
                qBtn.onClick.AddListener(() => QClick(skill));
                eBtn.onClick.RemoveAllListeners();
                eBtn.onClick.AddListener(() => EClick(skill));
                spaceBtn.onClick.RemoveAllListeners();
                spaceBtn.onClick.AddListener(() => SpaceClick(skill));
            }
            else
            {
    ...
    private void QClick(SkillNode skill)
    {
        SkillSGT.Instance.Skill.SetCurrentSkill(skill, SkillButton.Q);
        OnSkillUsing?.Invoke();
    }

    private void EClick(SkillNode skill)
    {
        SkillSGT.Instance.Skill.SetCurrentSkill(skill, SkillButton.E);
        OnSkillUsing?.Invoke();
    }

    private void SpaceClick(SkillNode skill)
    {
        SkillSGT.Instance.Skill.SetCurrentSkill(skill, SkillButton.Space);
        OnSkillUsing?.Invoke();
    }
}