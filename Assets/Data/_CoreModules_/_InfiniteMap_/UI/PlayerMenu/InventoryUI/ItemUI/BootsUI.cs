...
public class BootsUI : GMono
{
    ...
    public void ShowBoots(InventoryEquip item)
    {
    ...
    private void Click()
    {
        if(!onSelectObject.gameObject.activeSelf)
        {
            if(Time.time - lastTimeClick < doubleClickTheshold)
            {
                ...
                EquipDetailsUI.Instance.ShowDetails(InfiniteMapManager.Instance.Inventory.WeaponList[index]);
            }
        ...
        if(BootsUISpawner.Instance.SelectedItem != null) BootsUISpawner.Instance.SelectedItem.gameObject.SetActive(true);
        onSelectObject.gameObject.SetActive(false);

        BootsUISpawner.Instance.SelectedItem = onSelectObject;
    }
}