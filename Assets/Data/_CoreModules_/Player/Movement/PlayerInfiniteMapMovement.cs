...
public class PlayerIinfiniteMapMovement : PlayerComponent
{
    ...
    {
        if (isJumping && Player.InfiniteMapRb2D.velocity.y < 0)
        {
            isJumping = false;
            isFalling = true;
            Player.Anim.Fall();
    ...
    public void Move()
    {
        Player.InfiniteMapRb2D.velocity = new Vector2(InputManager.Instance.Horizontal * speed, Player.InfiniteMapRb2D.velocity.y);
        Vector2 distanceMoved2D = Player.InfiniteMapRb2D.velocity * Time.deltaTime;
    ...
    public void Jump()
    {
        ...
        Player.InfiniteMapRb2D.velocity = new Vector2(Player.InfiniteMapRb2D.velocity.x, jumpForce);
        isJumping = true;
        isFalling = false;
        isGrounded = false;
