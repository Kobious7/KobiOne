...
namespace InfiniteMap
{
    public class AuraUI : GMono
    {
        ...
        public void ShowAura(InventoryEquip item)
        {
        ...
        private void Click()
        {
            if(!onSelectObject.gameObject.activeSelf)
            {
                if(Time.time - lastTimeClick < doubleClickTheshold)
                {
                    ...
                    EquipDetailsUI.Instance.ShowEquip(Game.Instance.Inventory.AuraList[index]);
                }
            ...
            if(AuraUISpawner.Instance.SelectedItem != null) AuraUISpawner.Instance.SelectedItem.gameObject.SetActive(true);
            onSelectObject.gameObject.SetActive(false);

            AuraUISpawner.Instance.SelectedItem = onSelectObject;
        }
    }
}